...
/*
 ...
 3. Current model absolute index: 1
...
class ScrollEngine_3_Tests : ScrollEngineBase {
    
    let expectedNumberOfModels          = 5
    let expectedRelativeIndices         = [-1,0,1,2,3]
    ...
    let expectedPreviousXPosition       = CGFloat(0)
    let expectedYPosition               = CGFloat(0)
    
    override func setUp() {
        super.setUp()
        
        absoluteIndex                = test_3_config.absoluteIndex
        viewsCount                   = test_3_config.numberOfViews
