...
/*
 ...
 3. Current model absolute index: 3
 4. Current model relative index: RelativeIndex.current
 5. Current model view position x: 300.0
 6. Next model view position x: 400.0
 7. Appended index: nil
 8. Previous model view position x: 200.0
 9. Appended index: nil
 10. Model view position y: 0.0
 
 */

class ScrollEngine_5_Tests : ScrollEngineBase {
    
    let expectedNumberOfModels          = 5
    let expectedRelativeIndices         = [-3,-2,-1,0,1]
    let expectedCurrentRelativeIndex    = RelativeIndex.current
    let expectedCurrentXPosition        = CGFloat(300)
    let expectedNextXPosition           = CGFloat(400)
    let expectedPreviousXPosition       = CGFloat(200)
    let expectedYPosition               = CGFloat(0)
    
    override func setUp() {
        super.setUp()
        
        absoluteIndex                = test_5_config.absoluteIndex
        viewsCount                   = test_5_config.numberOfViews
    ...
    func test_01_initialization() {
        ...
        XCTAssertEqual(currentModel?.absoluteIndex, absoluteIndex, "\nTest Failed: Expected `\(absoluteIndex)` absolute index but given \(currentModel?.absoluteIndex ?? -9999)\n")
